...
static int binlog_relay_applier_before_dispatch_event(void * param) {
  ...
  if(!(xid_info.init_xid_info(xid_str.c_str()))){
    LogPluginErr(ERROR_LEVEL,ER_LOG_PRINTF_MSG,
                 "Init xid_info failed, xid_str is %s",
                 xid_str.c_str());
  }
...
Binlog_relay_IO_observer relay_io_observer = {
    ...
    nullptr,nullptr,nullptr,nullptr,
    nullptr,binlog_relay_applier_before_dispatch_event
};
...
static int global_consistent_apply_plugin_init(MYSQL_PLUGIN plugin_info) {
  ...
  Retval_Status retval = consistent_view->init_and_parse(clog_path);
  if(retval != OK_STOP){
    LogPluginErr(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
...
static MYSQL_SYSVAR_STR(commit_log_path, 
                        clog_path_value,
...
static SYS_VAR *system_var[]={
  MYSQL_SYSVAR(check),
  MYSQL_SYSVAR(commit_log_path),
  NULL
};
